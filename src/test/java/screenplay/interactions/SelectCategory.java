...
public class SelectCategory implements Interaction {
    ...
    public SelectCategory(Target target) {
        this.target = target;
    }

    @Override
    @Step("{0} clicks category target")
    public <T extends Actor> void performAs(T actor) {
        actor.attemptsTo(Click.on(target));
    }

    public static Performable openDropdown(Target trigger) {
        return instrumented(SelectCategory.class, trigger);
    }

    public static Performable option(Target option) {
        return instrumented(SelectCategory.class, option);
    }
}
